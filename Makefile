.PHONY: all
all:
	OUT="Makefile.real" FDISK=/opt/homebrew/Cellar/util-linux/2.40.4/sbin/fdisk ./configure
...
run-rv:
	qemu-system-riscv64 -machine virt -kernel kernel-rv -m 1G -nographic -bios default \
		-drive file=/Users/shao/Downloads/sdcard-rv.img,if=none,format=raw,id=x0 \
